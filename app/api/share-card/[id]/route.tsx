...
// Server-side PNG using next/og (Satori).
// Satori rules: NO position:absolute/relative, NO overflow:hidden, pure flexbox only.
...
const SPECIES_EMOJI = {
  Cattle:'🐄',Buffalo:'🐃',Sheep:'🐑',Goat:'🐐',Dog:'🐕',Cat:'🐈',Poultry:'🐓',Horse:'🐴',Calf:'🐮',
}

// ── Shared sub-components — ALL Satori-safe (no position:absolute/relative) ───
...
function ImgBox({ uri, w, h, c, emoji='🧴' }) {
  return (
    <div style={{ width:w, height:h, flexShrink:0, borderRadius:12, background:`linear-gradient(145deg,${c.pale},white)`, border:`2px solid ${c.primary}30`, display:'flex', alignItems:'center', justifyContent:'center' }}>
      {uri
        ...
        : <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
            ...
            <span style={{ fontSize:7, color:c.primary, marginTop:3, fontFamily:'Barlow Condensed' }}>COMING SOON</span>
          </div>
      }
    </div>
  )
...
function ImgBoxRound({ uri, w, c, emoji='💊' }) {
  return (
    <div style={{ width:w, height:w, flexShrink:0, borderRadius:w/2, background:`linear-gradient(145deg,${c.pale},white)`, border:`2px solid ${c.primary}30`, display:'flex', alignItems:'center', justifyContent:'center' }}>
      {uri
...
function DescBar({ desc, tags, c }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', padding:'10px 18px 8px', background:c.pale, borderBottom:`1.5px solid ${c.primary}18` }}>
    ...
    <div style={{ display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:'linear-gradient(135deg,#FFE600 0%,#FFD000 50%,#FFE600 100%)', padding:'14px 20px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end' }}>
          <span style={{ fontFamily:'Barlow Condensed', fontSize:9, color:'#111', fontWeight:800 }}>I.S.O. 9001:2013 COMPANY</span>
          <span style={{ fontFamily:'Barlow Condensed', fontSize:8, color:'#333', marginTop:2 }}>madvet.animal@gmail.com</span>
          <span style={{ fontFamily:'Barlow Condensed', fontSize:8, color:'#333' }}>www.madvet.in · support@madvet.in</span>
...
function CardVitality({ p, c, productImg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', width:480, background:'#fff', fontFamily:'Barlow Condensed' }}>
      {/* HERO */}
      <div style={{ display:'flex', flexDirection:'column', background:`linear-gradient(135deg,${c.darkest} 0%,${c.primary} 55%,${c.bright} 100%)`, padding:'18px 20px 18px' }}>
      ...
      {/* Gold tagline */}
      <div style={{ display:'flex', padding:'7px 24px', background:`linear-gradient(90deg,${c.darkest}22,transparent)` }}>
      ...
      {/* Body: benefits + image */}
      <div style={{ display:'flex', padding:'14px 16px 6px', gap:14 }}>
        <div style={{ display:'flex', flexDirection:'column', flex:1 }}>
          {hi.slice(0, 6).map((b, i) => {
            const big = i === 0 || i === 1 || i === 3 || i === 5
            return (
              <div key={i} style={{ display:'flex', marginBottom:big?7:5 }}>
                <div style={{ flex:1, background:big?`linear-gradient(90deg,${c.darkest},${c.primary})`:`linear-gradient(90deg,${c.primary},${c.mid})`, borderRadius:6, padding:big?'9px 14px':'6px 12px' }}>
                  ...
                  {en[i] && <span style={{ fontSize:8.5, color:'rgba(255,255,255,0.58)', fontFamily:'Barlow Condensed', marginTop:2, display:'block' }}>{en[i]}</span>}
                </div>
              </div>
            )
          })}
...
function CardDigest({ p, c, productImg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', width:480, background:'#fff', fontFamily:'Barlow Condensed' }}>
      {/* HEADER: text left, image right — pure flex row, no position:absolute */}
      ...
      <div style={{ display:'flex', padding:'12px 20px', gap:14 }}>
        <div style={{ display:'flex', flexDirection:'column', flex:1 }}>
          ...
          {hi.slice(0, 6).map((b, i) => (
            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:7, padding:'6px 10px', borderRadius:6, background:i%2===0?c.pale:'transparent', borderLeft:`3px solid ${i%2===0?c.primary:c.bright}` }}>
...
function CardHerbal({ p, c, productImg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', width:480, background:'#fff', fontFamily:'Barlow Condensed' }}>
      <div style={{ display:'flex', flexDirection:'column', background:`linear-gradient(160deg,${c.darkest} 0%,${c.primary} 60%,${c2} 100%)`, padding:'16px 20px 18px' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', padding:'14px 18px 8px' }}>
        ...
        <DescBar desc={desc} tags={tags} c={c} />
        {/* 2-column grid using flex rows */}
        {[0, 2, 4].map(rowStart => (
          <div key={rowStart} style={{ display:'flex', gap:7, marginTop:7 }}>
            {[rowStart, rowStart+1].map(i => {
              const b = hi[i]
              if (!b) return <div key={i} style={{ display:'flex', flex:1 }} />
              return (
                <div key={i} style={{ display:'flex', gap:8, padding:'7px 10px', background:i<2?c.pale:'#fafafa', borderRadius:7, border:`1px solid ${i<2?c.primary+'33':'#eeeeee'}`, alignItems:'flex-start', flex:1 }}>
...
function CardShield({ p, c, productImg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', width:480, background:'#fff', fontFamily:'Barlow Condensed' }}>
      <div style={{ display:'flex', flexDirection:'column', background:`linear-gradient(125deg,${c.darkest} 0%,${c.primary} 100%)`, padding:'18px 20px 22px' }}>
...
function CardClinical({ p, c, productImg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', width:480, background:'#fff', fontFamily:'Barlow Condensed' }}>
      <div style={{ display:'flex', flexDirection:'column', background:`linear-gradient(135deg,hsl(${c.h},${c.s}%,${c.l-18}%) 0%,hsl(${c.h},${c.s}%,${c.l-10}%) 50%,${c.primary} 100%)`, padding:'16px 20px 20px' }}>
...
export async function GET(_req, { params }) {
  ...
  const height = Math.min(880, Math.max(640, 220 + numBenefits * 46 + 280))
  ...
  try {
    ...
    return new ImageResponse(
      <div style={{ display:'flex', width:480, height:120, background:'#fee2e2', alignItems:'center', justifyContent:'center', padding:24 }}>
        <span style={{ fontSize:13, color:'#991b1b', fontFamily:'sans-serif' }}>Error: {String(err?.message||err).slice(0,120)}</span>
      </div>,
