// @ts-nocheck
// app/api/share-card/[id]/route.tsx — NO fonts, system fonts only, guaranteed working
...
const CAT_PALETTES = {
  'Vitamin Supplement': {h:22,s:85,l:32}, 'Vitamin Supplement / Galactogogue': {h:210,s:80,l:28},
  'Antibiotic': {h:218,s:72,l:26}, 'Anti-inflammatory / Analgesic': {h:338,s:78,l:30},
  'Anthelmintic / Antiparasitic': {h:158,s:70,l:26}, 'Probiotic': {h:128,s:65,l:28},
  'Dermatological': {h:272,s:60,l:30}, 'Ectoparasiticide': {h:42,s:80,l:30},
  'Reproductive Hormone': {h:295,s:58,l:28}, 'Antihistamine': {h:200,s:68,l:26},
  'Antidiarrheal': {h:168,s:65,l:26}, 'Udder Care / Herbal Antimicrobial': {h:88,s:62,l:28},
}
const CAT_NORMALIZE = {
  'Anti-inflammatory':'Anti-inflammatory / Analgesic','Anti-inflammatory, Analgesic, Antipyretic':'Anti-inflammatory / Analgesic',
  'Anti-inflammatory / Analgesic / Antipyretic':'Anti-inflammatory / Analgesic','Analgesic / Antipyretic':'Anti-inflammatory / Analgesic',
  'Analgesic, Antipyretic':'Anti-inflammatory / Analgesic','Analgesic':'Anti-inflammatory / Analgesic',
  'Anthelmintic':'Anthelmintic / Antiparasitic','Antiparasitic':'Anthelmintic / Antiparasitic',
  'Antibiotic (Cephalosporin)':'Antibiotic','Antibiotic (Fluoroquinolone)':'Antibiotic',
  'Antihistamine / Anti-allergic':'Antihistamine','Dermatological / Topical':'Dermatological',
  'Probiotic / Immunomodulator / Vitamin Supplement':'Probiotic','Antidiarrheal / Gastrointestinal':'Antidiarrheal',
}
...
function getColors(id,cat){
  const base=CAT_PALETTES[cat]??{h:220,s:70,l:28},shift=((id*37+13)%41)-20,h=(base.h+shift+360)%360,{s,l}=base
  ...
  return{primary:`rgb(${r},${g},${b})`,bright:`rgb(${rb},${gb},${bb})`,dark:`rgb(${rd},${gd},${bd})`,darkest:`rgb(${rdk},${gdk},${bdk})`,pale:`hsl(${h},${s-20}%,95%)`,mid:`rgb(${rm},${gm},${bm})`,p12:`rgba(${r},${g},${b},0.12)`,p15:`rgba(${r},${g},${b},0.15)`,p20:`rgba(${r},${g},${b},0.20)`,p25:`rgba(${r},${g},${b},0.25)`,p30:`rgba(${r},${g},${b},0.30)`,p40:`rgba(${r},${g},${b},0.40)`,dk20:`rgba(${rdk},${gdk},${bdk},0.20)`}
}
...
const HI_IND={fever:'बुखार में असरदार',pain:'दर्द से जल्दी राहत',inflammation:'सूजन कम करे',infection:'संक्रमण से लड़े',mastitis:'थनिका में कारगर',diarrhea:'दस्त रोकने में कारगर',respiratory:'श्वसन रोग में राहत',skin:'त्वचा रोग में लाभकारी',milk:'दूध उत्पादन बढ़ाए',vitamin:'विटामिन की कमी दूर करे',worm:'कृमि खत्म करे',calcium:'कैल्शियम की कमी पूरी करे'}
function augBenefits(hiL,enL,ind='',min=4){
  if(hiL.length>=min)return{hi:hiL,en:enL}
  const need=min-hiL.length,nhi=[],nen=[]
  for(const term of (ind||'').split(',').map(s=>s.trim().toLowerCase()).filter(s=>s.length>2)){
    if(nhi.length>=need)break
    const e=Object.entries(HI_IND).find(([k])=>term.includes(k))
    if(e&&!hiL.includes(e[1])){nhi.push(e[1]);nen.push(term)}
  }
  return{hi:[...hiL,...nhi].slice(0,5),en:[...enL,...nen].slice(0,5)}
}
function getDesc(d='',max=130){if(!d)return'';const f=d.split(/\.\s+/)[0];const t=f.length<=max?f:f.slice(0,max).replace(/\s\S+$/,'')+'…';return t.endsWith('.')?t:t+'.'}
...
function Card({p,c,productImg,logoImg}){
  const hi=splitBSafe(p.usp_benefits_hi,p.benefits),en=splitB(p.benefits)
  const{hi:benefits,en:benefitsEn}=augBenefits(hi,en,p.indication)
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',width:480,background:'#fff'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',background:`linear-gradient(135deg,${c.darkest} 0%,${c.primary} 55%,${c.bright} 100%)`,padding:'18px 22px 20px'}}>
        ...
        {benefits[0]&&<div style={{display:'flex',marginTop:12,alignSelf:'flex-start'}}><div style={{background:'#FFE000',borderRadius:6,padding:'6px 14px',display:'flex'}}><span style={{fontSize:14,fontWeight:800,color:c.darkest}}>{benefits[0]}</span></div></div>}
      </div>
      {/* DESC BAR */}
      {(desc||tags.length>0)&&<div style={{display:'flex',flexDirection:'column',padding:'10px 20px 8px',background:c.pale}}>
        ...
        {tags.length>0&&<div style={{display:'flex',gap:5,flexWrap:'wrap',alignItems:'center'}}>
          <span style={{fontSize:9,color:c.primary,fontWeight:800,letterSpacing:1}}>TREATS:</span>
          {tags.map((t,i)=><div key={i} style={{display:'flex',fontSize:9,color:c.dark,background:c.p12,borderRadius:20,padding:'2px 8px'}}><span>{t}</span></div>)}
        </div>}
      </div>}
      {/* BENEFITS + IMAGE */}
          ...
          {benefits.slice(1).map((b,i)=>(
            <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'7px 12px',borderRadius:8,background:i===0?c.pale:i%2===0?'#f8f8f8':'#fff',marginBottom:6,borderLeftWidth:3,borderLeftStyle:'solid',borderLeftColor:i===0?c.primary:c.bright,borderTopWidth:0,borderRightWidth:0,borderBottomWidth:0}}>
              ...
              <div style={{display:'flex',flexDirection:'column',flexGrow:1,flexShrink:1,flexBasis:0}}>
                ...
                {benefitsEn[i+1]&&<span style={{fontSize:10,color:'#888',marginTop:1}}>{benefitsEn[i+1]}</span>}
              </div>
            </div>
          ))}
        </div>
        <div style={{width:120,flexShrink:0,display:'flex',flexDirection:'column',alignItems:'center',gap:8,paddingRight:16,paddingTop:4}}>
          ...
          <div style={{display:'flex',gap:4,flexWrap:'wrap',justifyContent:'center'}}>
            {(p.species||'').split(/[,\/]/).map(s=>s.trim()).filter(Boolean).slice(0,4).map((s,i)=>(
              <div key={i} style={{width:26,height:26,borderRadius:13,background:c.p15,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>
                <span>{SP[s]||'🐾'}</span>
              </div>
            ))}
          </div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'#FFD700',padding:'12px 20px'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{background:'#fff',borderRadius:8,padding:'3px 5px',display:'flex'}}>
            {logoImg?<img src={logoImg} width={36} height={36} style={{width:36,height:36,objectFit:'contain'}}/>:<span style={{fontSize:28}}>🐾</span>}
          </div>
          <div style={{display:'flex',flexDirection:'column'}}>
            ...
            <span style={{fontSize:8,color:'#555'}}>Ghaziabad (U.P.)</span>
          </div>
        </div>
        <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end'}}>
