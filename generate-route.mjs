// Run this on your machine: node generate-route.mjs
// It downloads fonts, embeds them as base64, and writes the final route.tsx

import { readFileSync, writeFileSync, mkdirSync } from 'fs'

const fonts = [
  { name: 'Oswald',               weight: 700, file: 'oswald-700',  url: 'https://fonts.gstatic.com/s/oswald/v53/TK3_WkUHHAIjg75cFRf3bXL8LICs13NvgUFoZAaRliE.woff2' },
  { name: 'Barlow Condensed',     weight: 600, file: 'barlow-600',  url: 'https://fonts.gstatic.com/s/barlowcondensed/v12/HTxwL3I-JCGChYJ8VI-L6OO_au7B43LT31vzHvs.woff2' },
  { name: 'Barlow Condensed',     weight: 700, file: 'barlow-700',  url: 'https://fonts.gstatic.com/s/barlowcondensed/v12/HTxxL3I-JCGChYJ8VI-L6OO_au7B4-Lz3wFI9ZCb.woff2' },
  { name: 'Barlow Condensed',     weight: 800, file: 'barlow-800',  url: 'https://fonts.gstatic.com/s/barlowcondensed/v12/HTxzL3I-JCGChYJ8VI-L6OO_au7B44oQ0TrZnZWI.woff2' },
  { name: 'Noto Sans Devanagari', weight: 600, file: 'noto-600',    url: 'https://fonts.gstatic.com/s/notosansdevanagari/v25/TuGoUUFzXI5FBtUq5a8bjKYTZjtgrbn0_-AHR5-tSg.woff2' },
  { name: 'Noto Sans Devanagari', weight: 700, file: 'noto-700',    url: 'https://fonts.gstatic.com/s/notosansdevanagari/v25/TuGoUUFzXI5FBtUq5a8bjKYTZjtgrbn0_-AHR5-vSQ.woff2' },
  { name: 'Noto Sans Devanagari', weight: 800, file: 'noto-800',    url: 'https://fonts.gstatic.com/s/notosansdevanagari/v25/TuGoUUFzXI5FBtUq5a8bjKYTZjtgrbn0_-AHR5-lSg.woff2' },
]
...
for (const f of fonts) {
  ...
  fontData[f.file] = Buffer.from(buf).toString('base64')
  console.log(`  ${f.file}: ${(buf.byteLength/1024).toFixed(0)}KB`)
}

const fontSnippet = fonts.map(f =>
  `  { name: '${f.name}', weight: ${f.weight}, style: 'normal' as const, data: Buffer.from('${fontData[f.file]}', 'base64') },`
).join('\n')
...
async function imgURI(url) {
  if (!url) return null
  try {
    const fetchUrl = url.includes('supabase') ? url + '?width=400&quality=90' : url
...
writeFileSync('app/api/share-card/[id]/route.tsx', route)
console.log('\\n✅ route.tsx written successfully!')
console.log('Now run: git add . && git commit -m "feat: fonts embedded as base64" && git push')
